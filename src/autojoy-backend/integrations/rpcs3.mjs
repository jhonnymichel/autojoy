...
const inputConfigFileName = resolveConfigPath(`input_configs/global/${inputProfile}.yml`);
const activeProfileFileName = resolveConfigPath(`input_configs/active_input_configurations.yml`);
...
function resolveConfigPath(file) {
  try {
    ...
    console.log(
      "RPCS3 - installation doesn´t appear to have a config/ folder. using root folder.\n",
      e.message
    );
